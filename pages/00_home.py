import solara
import leafmap.maplibregl as leafmap  # 使用 maplibregl 後端，這對 3D 支援更好

# 設定頁面標題
# 這裡設定的 title 會顯示在瀏覽器分頁標籤上
@solara.component
def Page():
    
    # --- 1. 頁面標題區 ---
    with solara.Column(style={"padding": "20px"}):
        solara.Title("橫貫台灣：從平地到雲端") # 瀏覽器標籤
        ...
        # --- 2. 關鍵地點展示 (模擬資料) ---
        # 這裡我們先用簡單的 Markdown 表格或卡片來介紹
        # 之後可以改成從 CSV 讀取並顯示在 Map 上
    ...
    # --- 3. 互動地圖預覽 ---
    # 這裡我們先放一個簡單的 2D 地圖，定位在台灣中心
    # 之後我們會換成有 GPX 軌跡的版本
    with solara.Column(style={"padding": "20px", "height": "600px"}):
        solara.Markdown("### 🗺️ 路線概覽")
        
        # 建立地圖物件
        # style="positron" 是一個很乾淨的底圖，適合疊加資料
        m = leafmap.Map(center=[24.0, 121.1], zoom=9, style="positron")
        
        # 這裡示範加一個 Marker (標記) 在武嶺
        m.add_marker([121.276, 24.137], title="武嶺 (3275m)")
        
        # 顯示地圖
        # 這是 Solara 顯示 ipywidgets 的標準寫法
        m.element() 
...
# 這行是讓 Solara 知道這是主要元件（雖然在 pages 模式下不是必須，但好習慣）
Page()